...
class BrassPlayers
{
	...
	private int active_player_id; //is the player who's turn it is
	...
	public BrassPlayers(BrassDeck brass_deck, BrassXML brass_xml)
	{
		...
		for (int i = 1; i <= 8; i++)
		{
		    for (int j = 0; j < brass_players.size(); j++)
            {
                brass_players.get(j).addCardToHand(brass_deck.deal());
            }
		}
   ...
   {	
	    ...
	    for (int i = 0; i < brass_players.size(); i++)
        {
            brass_players.get(i).draw(g, active_player_id, view_player_id);
        }
   }
}

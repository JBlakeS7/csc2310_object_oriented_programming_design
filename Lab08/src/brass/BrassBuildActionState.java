...
class BrassBuildActionState implements BrassState
{
	...
	//DO THIS
	//need a setSelectedCard method to store the forwarded information
	public void setCardSelected(int city_tech_id)
	{
		brass_card_city_tech_id = city_tech_id;
	}
	...
	public void mouseClicked(int x_click, int y_click)
	{
		...
		{
			//DO THIS
			//get the industry_id clicked on (getSelectedToken in BrassGame), get the city_id from brass_card_city_tech_id
			//if the industry_id > 0, test to see if the build can proceed
		...
		{
			//DO THIS
			//get the city_id clicked on, subtract 19 from brass_card_city_tech_id to get the industry_id
			//if the city_id > 0, test to see if the build can proceed

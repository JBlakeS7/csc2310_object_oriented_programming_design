...
class BrassBoard
{
	...
	{
		//DO THIS
		//check to see if the player is present in the city (method in BrassCities)
		//or if player has a link into the city (method in BrassConnections)
		boolean player_in_city = (brass_cities.getNumTokensInCity(city_id, player_id) > 0);
		boolean player_has_link_into_city = brass_connections.doesPlayerHaveLinkIntoCity(city_id, player_id);
		
		if(player_in_city || player_has_link_into_city;)
			return true;
	...
	{
		//DO THIS
		//call the above method to see if either city is part of the player's network

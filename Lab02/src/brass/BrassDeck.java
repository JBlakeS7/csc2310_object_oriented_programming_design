...
class BrassDeck
{
	private ArrayList<BrassCard> brass_deck;  		//keep the original deck intact as there are two phases
	private ArrayList<BrassCard> shuffled_deck;
	...
	private ArrayList<BrassCard> shuffle()
	{
		...
		Permutation permutation = PermutationFactory.getPermutation("randPermOutput.txt", deckSize, deckSize);
		shuffled_deck = new ArrayList<BrassCard>();
		
		while(permutation.hasNext())
		{
			shuffled_deck.add(brass_deck.get(permutation.next()-1));
		}
	...
	{
		return shuffled_deck.remove(shuffled_deck.size()-1);
		//return null;
	}
	...
	{
		for(BrassCard brass_card : brass_deck)
		{
			brass_card.draw(g);
		}
	}

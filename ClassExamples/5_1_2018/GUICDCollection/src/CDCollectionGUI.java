...
public class CDCollectionGUI extends JFrame
{
    ...
    private JButton btnSort, btnAll, btnAdd, btnDefault, btnFirst, btnLast,
    ...
    {
        this("resources/cds.txt");
    }
    
    public CDCollectionGUI(String fileName)
    {
        super("CDCollectionGUI");
        icon = new ImageIcon("resources/Scruffy.jpg"); // just for fun, picture on upper left corner of frame
    ...
    //    this.setEnabled(false); // experiment, DON'T DO IF YOU WANT A USABLE GUI
    }
    ...
    {
        locListener = new LocationListener(theCollection.getSize() / 2);
        btnDefault = new JButton("Default");
        btnDefault.addActionListener(locListener);
    ...
    private class LocationListener implements ActionListener
    {
        ...
        public LocationListener(int location) // 1-based for caller
        {
            if (location < 1 || location > theCollection.getSize())
            {
                location = 1;
            }
        ...
        public void resetLocation()
        {
            ...
            {
                updateTextFields();
                infoArea.append("Displaying CD at position " + (defaultLocation + 1) + "\r\n");
            }
        }
        ...
        public void actionPerformed(ActionEvent e)
        {
            ...
            {
              System.out.println("btnDefault clicked");
              location = defaultLocation;
            }
            ...
            {
                updateTextFields();
                infoArea.append("Displayed CD information for the CD at position " + (location + 1) + "\r\n");
            }
            else
            {
    ...
    private class SaveListener implements ActionListener
    {
        public void actionPerformed(ActionEvent e)
        {
            ...
            int returnVal = chooser.showSaveDialog(CDCollectionGUI.this); // instance of the outer class
            if (returnVal == JFileChooser.APPROVE_OPTION)
            {
                File file = chooser.getSelectedFile();
                String filename = file.getAbsolutePath(); // full path for if directory is changed
                try
                {
    ...
    private class ObliterateListener implements ActionListener
    {
        ...
        {
            System.out.println(e);
            infoArea.append("Beginning obliterate...\r\n");
            String choice = SimpleDialogs.stringInput("ARE YOU SURE??? (y/n)", "Possible permanent damage");
            if (choice.equalsIgnoreCase("y"))
            {
               infoArea.append("Obliterating collection.\r\n");
               theCollection.obliterate();
               clearAllTextFields();
            }
            else
            {
               infoArea.append("Not changing collection.\r\n");
            }
        }
    }

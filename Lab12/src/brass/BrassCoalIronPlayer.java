...
class BrassCoalIronComputerPlayer implements BrassComputerPlayer
{
	private BrassGame brass_game;
	private int num_upgrades_done=0;
	public BrassCoalIronComputerPlayer(BrassGame bg)
	{
		brass_game = bg;
	}
	...
	public BrassComputerPlayerAction getBrassMove()
	{
		...
		{	int rand_card = getCardForNonBuildAction(computer_player_id);
			
			if(num_upgrades_done <= 1)
			{	
				computer_move.selectTechUpgradeAction(rand_card,2,2);
				computer_move.selectTechUpgradeAction(rand_card,4,4);
				num_upgrades_done++;
				/*if(brass_game.getBrassPhase()==true)
				{
					computer_move.selectTechUpgradeAction(rand_card,2,2);
					computer_move.selectTechUpgradeAction(rand_card,4,4);
				}*/
		...
		int player_money = brass_game.getMoney(computer_player_id);
		int player_income = brass_game.getIncome(computer_player_id);
		if (player_money < 14 && player_income <= 5)
		{
			computer_move.selectTakeLoanAction(getCardForNonBuildAction(computer_player_id), 3);
			return computer_move;
		}

...
public class BrassGame implements Drawable, util.GameAccept
{
	...
	//DO THIS
	//declare a game_reader and a game_writer
	private util.GameVisitor game_reader;
	private util.GameVisitor game_writer;
	
	public void keyPressed(char key) 
	{
		//DO THIS 
		//call accept where appropriate
		
		//save a game (use game_writer)
		if (key == 's')
		{
			//a game can only be saved in the select card state or the game over state
			//also, it must be a player's first action
			if (current_state != select_card_state && current_state != game_over_state) return;
			if (!brass_players.isFirstAction()) return;
			accept(game_writer);
		}
		//load a game (use game_reader)
		else if (key == 'l')
		{
			accept(game_writer)
			//need to update the player actions
			brass_gui.repaint();
		}
	}
	
	//DO THIS
	//implement the accept method as appropriate for the visitor design pattern
	public void accept(util.GameVisitor visitor)
	{
		visitor.visit(this);
	}
	
	public BrassGame(String game_file_name, int num_players)
   {
	   //DO THIS
	   game_reader = new util.GameReadFileVisitor(game_file_name);
	   game_writer = new util.GameWriteFileVisitor(game_file_name); 
	...
	public org.jdom2.Document writeGameXML()
	{
		org.jdom2.Element brass_element = new org.jdom2.Element("brass");
		
		org.jdom2.Element over_element = new org.jdom2.Element("over");
		if (current_state == game_over_state)
		{
			over_element.setText("" + 1);
		}
		else
		{
			over_element.setText("" + 0);
		}
		brass_element.addContent(over_element);
		
		org.jdom2.Element phase_element = new org.jdom2.Element("phase");
		if (brass_phase == true)
		{
			phase_element.setText("" + 1);
		}
		else
		{
			phase_element.setText("" + 0);
		}
		brass_element.addContent(phase_element);
		
		org.jdom2.Element players_element = brass_players.writeXML();
		brass_element.addContent(players_element);
		
		org.jdom2.Element board_element = brass_board.writeXML();
		brass_element.addContent(board_element);
		
		org.jdom2.Element shuffled_deck_element = brass_deck.writeXML();
		brass_element.addContent(shuffled_deck_element);
		
		org.jdom2.Document doc = new org.jdom2.Document(brass_element);
		return doc;
   }
   
   public void readGameXML(org.jdom2.Document doc)
   {
		org.jdom2.Element brass_element = doc.getRootElement();

		org.jdom2.Element over_element = brass_element.getChild("over");
		int over = Integer.parseInt(over_element.getText());
		if (over == 1) 
		{
		...
		org.jdom2.Element phase_element = brass_element.getChild("phase");
		int phase = Integer.parseInt(phase_element.getText());
		
		brass_players.readXML(brass_element.getChild("players"), brass_phase, brass_deck, brass_track);
		if (phase == 1) 
		{
			brass_phase = true;
			brass_board.activateRails();
			brass_players.changePhase();  //must be called here
		}
		else
		{
			brass_phase = false;
			brass_players.changePhaseBack();
		}
		
		brass_board.readXML(brass_element.getChild("board"), brass_phase, brass_players);
		brass_deck.readXML(brass_element.getChild("deck"));
   }
   
   public boolean isFirstAction()
	{
		return brass_players.isFirstAction();
	}

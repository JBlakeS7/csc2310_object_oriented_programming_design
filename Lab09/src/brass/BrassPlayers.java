...
class BrassPlayers
{
	...
	public void playerActionCompleted(BrassDeck brass_deck)
	{
		...
		if (turn_count == 5)  //all four players have taken a turn
		{
			...
			Comparator<BrassPlayer> compare = new BrassCompareAmountSpent(true);
			turn_order = PQSort.pqSort(turn_order, compare, pqsort.PQType.TREE);
			...
			{
				BrassPlayer player = turn_order_iter.next();
				player.resetAmountSpent();
				player.receiveIncome();
				player.setTurnOrderImageLoc(turn_order_locations.get(count-1).getX(),turn_order_locations.get(count-1).getY());
				count++;
			}
		}

   ...
   public void mouseClicked(int x_click, int y_click)
	{
		...
		//Checks to see if phase needs to be changed and changes it
		if(brass_board.isChangePhaseSelected(x_click, y_click)) 
			brass_players.changePhase();

		// Check to see if the user clicked on a grey connection image to construct a link. If so,
		// construct the link (BrassBoard method) and pay for the link (BrassPlayers method). 
		if(brass_board.getSelectedLink(x_click, y_click)>0 && brass_players.canPayForLink(brass_phase, active_player_id))
		{
			// May need another check to see if the link is grey so the player isn't charged
			// for clicking on a link that is already constructed
			brass_board.constructLink(brass_board.getSelectedLink(x_click, y_click), active_player_id);
